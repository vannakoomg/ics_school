...
<script>
 ...
 $("*").dblclick(function(e){
  ...
  $(document).on('click', '#closed_status', function (e) {
     clearInterval(timmer);
     table.ajax.url("<?php echo e(route('admin.dlp.getdlp'), false); ?>?campus=" + selectCampus + "&class_id=" + selectClassId + '&closed_status=' + $('#closed_status').is(':checked')).load();
     timmer = setInterval(main_process,9000);
  });
  ...
  var data = {'dlp_id': btn.data('id'), 'solution': solution , 'status':status ,"_token": "<?php echo e(csrf_token(), false); ?>"};

    $.ajax({
        type: 'post',
        url : "<?php echo e(route('admin.dlp.update'), false); ?>",
        data: data,
        dataType: 'json',
        success: function(data){
           if(data.success){
                //table.ajax.url("<?php echo e(route('admin.dlp.getdlp'), false); ?>?class_id=" + selectClassId + '&closed_status=' + $('#closed_status').is(':checked')).load();
                main_process();
                timmer = setInterval(main_process,9000);
           }else{
...
//   $('.btn_addnew').click(function(){
    ...
    var data = {'class_id': $('#txt_class_id').val(), 'problem_type': $('#lst_type').val(), 'problem': prob ,"_token": "<?php echo e(csrf_token(), false); ?>"};

    $.ajax({
        type: 'post',
        url : "<?php echo e(route('admin.dlp.addnew'), false); ?>",
        data: formData,
        contentType: false,
        processData: false,
        dataType: 'json',
		...
		$(':input[type="submit"]').prop('disabled', false);
                ...
                //DesktopNotify.send('<?php echo Session::get('notifier.notice'); ?>');

                table.ajax.url("<?php echo e(route('admin.dlp.getdlp'), false); ?>?campus=" + selectCampus + "&class_id=" + selectClassId).load();
                timmer = setInterval(main_process,9000);
           }else{
                console.log('Error Update');
           }
   ...
   table = $('.dlp-table').DataTable({
        ...
        paging: false,
        ajax: {
            url:"<?php echo e(route('admin.dlp.getdlp'), false); ?>?campus=" + selectCampus + "&class_id=" + selectClassId,
 ...
 // } //end function table
  ...
  var main_process = function(){
    ...
    return $.ajax({
    type: 'get',
    url : "<?php echo e(route('admin.dlp.getclass'), false); ?>",
...
<?php echo $__env->make('laravelPnotify::notify', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
            ...
            <div class="card">
                ...
                <div class="row">
                    ...
                    <div class="col-lg text-center">
                        <a href="<?php echo e(route('admin.dlp.report'), false); ?>" class="btn btn-outline-primary">DLP Report</a>
                    </div>
                ...
                <form method="POST" enctype="multipart/form-data" id="formsubmit" action="javascript:void(0)" >
                    ...
                    <div class="form-group">
                         ...
                         <select name='lst_type' id='lst_type' class="form-control">
                            <?php $__currentLoopData = $problem_types; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $type): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /mnt/wwww/school/resources/views/admin/dlp.blade.php ENDPATH**/ ?>