...
<script>
 ...
 $("*").dblclick(function(e){
  ...
  $(document).on('click', '#closed_status', function (e) {
     clearInterval(timmer);
     table.ajax.url("<?php echo e(route('admin.dlp.getdlp')); ?>?campus=" + selectCampus + "&class_id=" + selectClassId + '&closed_status=' + $('#closed_status').is(':checked')).load();
     timmer = setInterval(main_process,9000);
  });
  ...
  var data = {'dlp_id': btn.data('id'), 'solution': solution , 'status':status ,"_token": "<?php echo e(csrf_token()); ?>"};

    $.ajax({
        type: 'post',
        url : "<?php echo e(route('admin.dlp.update')); ?>",
        data: data,
        dataType: 'json',
        success: function(data){
           if(data.success){
                //table.ajax.url("<?php echo e(route('admin.dlp.getdlp')); ?>?class_id=" + selectClassId + '&closed_status=' + $('#closed_status').is(':checked')).load();
                main_process();
                timmer = setInterval(main_process,9000);
           }else{
...
//   $('.btn_addnew').click(function(){
    ...
    var data = {'class_id': $('#txt_class_id').val(), 'problem_type': $('#lst_type').val(), 'problem': prob ,"_token": "<?php echo e(csrf_token()); ?>"};

    $.ajax({
        type: 'post',
        url : "<?php echo e(route('admin.dlp.addnew')); ?>",
        data: formData,
        contentType: false,
        processData: false,
        dataType: 'json',
		...
		$(':input[type="submit"]').prop('disabled', false);
                ...
                table.ajax.url("<?php echo e(route('admin.dlp.getdlp')); ?>?campus=" + selectCampus + "&class_id=" + selectClassId).load();
                timmer = setInterval(main_process,9000);
           }else{
                console.log('Error Update');
           }
   ...
   table = $('.dlp-table').DataTable({
        ...
        paging: false,
        ajax: {
            url:"<?php echo e(route('admin.dlp.getdlp')); ?>?campus=" + selectCampus + "&class_id=" + selectClassId,
 ...
 // } //end function table
  ...
  var main_process = function(){
    ...
    return $.ajax({
    type: 'get',
    url : "<?php echo e(route('admin.dlp.getclass')); ?>",
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="clearfix">
                    <div class="pull-left">DLP Dashboard
                    ...
                    Show Closed: <input type="checkbox" id="closed_status" value="1"/></div>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/icsapp/resources/views/admin/dlp.blade.php ENDPATH**/ ?>