...
<div class="card">
    ...
    <div class="card-body">
          ...
          <form name="frmtimetable" method="POST" action="{{ route("admin.timetable.store") }}" enctype="multipart/form-data">  
            @csrf
            ...
            <div class="row">  
              ...
              <div class="col-2">
                <label>&nbsp;</label>
                <a class="btn btn-info form-control" href="{{ route('admin.timetable.exporttimetable',['class_id' => $current_filter['class_id']]) }}">
                  Export Timetable
                </a>
              </div>
              ...
              <div class="col-2">  
                <div class="form-group">
                    <label class="required" for="start_time">{{ trans('cruds.lesson.fields.class') }}</label>
                    <label  class="form-control">{{ old('class_name', $current_filter['class_name']) }}</label>
                    <input type="hidden" name="class_id" value="{{ $current_filter['class_id'] }}"/>
                    <input type="hidden" name="campus" value="{{ $current_filter['campus'] }}"/>
                </div>
                </div>
                ...
                @endphp
                  ...
                  <select name="breaktime" class="custom-select">
                       @foreach ($breaktimes as $key=>$value)
                            <option value="{{ $value }}" {{ old('breaktime') == $value ? 'selected' : '' }}>{{ $key }}</option>
                       @endforeach
