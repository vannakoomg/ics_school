                ...
                <div class="col-sm-8">
                      <select  class="custom-select" name="filter_category" id="filter_category" onchange="document.forms['frmcampus'].submit()">
                          @foreach($categories as $id => $category)
                              <option value="{{ $category }}" {{ old('filter_category') == $category ? 'selected' : '' }} {{ $category==$current_filter['category']?'selected':'' }}>{{ $category }}</option>
                          @endforeach
                      </select>
                </div>         
              </div>
                        ...
                        <tr data-entry-id="{{ $course->id }}" bgcolor="{{ $course->color ?? '#FFF' }}">
                            ...
                            <td>
                                <div class="xzoom-thumbs text-left">
                                    @if(!empty($course->image))
                                        <img src="{{ asset('storage/image/' . $course->image) }}" class="xzoom-gallery img-thumbnail btn btn-outline-primary" id="img_thumbnail" xpreview="{{ asset('storage/image' . $course->image) }}" style="height: 100px !important">
                                    @else
                            ...
                            <td>
                                @can('course')
                                    ...
                                    <form action="{{ route('admin.course.destroy', $course->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
