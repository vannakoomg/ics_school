    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.course.update",$course->id) }}" enctype="multipart/form-data">
            ...
            <div class="form-group row">
                ...
                <div class="col-4">
                 ...
                 <select name="language" id="language" class="custom-select">
                  @foreach($languages as $id => $language)
                     <option value="{{ $language }}" {{ $course->language == $language ? 'selected' : '' }}>{{ $language }}</option>
                  @endforeach
                 </select>
                  @if($errors->has('name'))
                      <div class="invalid-feedback">
                ...
                <div class="col-2">
                    <img src="{{ empty($course->image)?asset('images/blank.jpeg'): asset('storage/image/' . $course->image) }}" class="img-thumbnail btn btn-outline-primary" id="img_thumbnail" alt="select thumbnail" width="150px" height="150px">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('class') ? 'is-invalid' : '' }}" name="class_id[]" id="class_id" multiple>
                    @foreach($classes as $id => $class)
                        @if(in_array($id, $filter[strtolower($course->category)]))
                        <option value="{{ $id }}" {{ (in_array($id, old('class_id', [])) || $course->class->contains($id))  ? 'selected' : '' }}>{{ $class }}</option>
                        @endif
                    @endforeach
                </select>
                @if($errors->has('class'))
            ...
            <div class="form-group">
                ...
                @foreach ($colors as $color)
                   <option value="{{ $color }}" {{ (old('color')==$course->color || $color==$course->color) ? 'selected':'' }}>{{ $color }}</option>
                @endforeach      
