    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.school-classes.update", [$schoolClass->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select name='campus' class="custom-select" required>
                    @foreach($campuses as $campus)
                        <option value="{{ $campus }}" {{ ($campus==$schoolClass->campus)?'selected':'' }}>{{ $campus }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="roomno" id="roomno" value="{{ old('roomno', $schoolClass->roomno) }}"/>
                <span class="help-block">{{ ''}}</span>
            </div>
                </div>
            </div>
            ...
            <select name='homeroom_id' class="custom-select">
                @foreach($teachers as $id => $teacher)
                    <option value="{{ $id }}" {{ ($id==$schoolClass->homeroom_id)?'selected':'' }}>{{ $teacher }}</option>
                @endforeach
            </select>
                <span class="help-block"></span>
            </div>
            ...
            <div class="form-group">
                ...
                <select name='khteacher_id' class="custom-select">
                    @foreach($teachers as $id => $teacher)
                        <option value="{{ $id }}" {{ ($id==$schoolClass->khteacher_id)?'selected':'' }}>{{ $teacher }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select name='teacheraide_id' class="custom-select">
                    @foreach($teachers as $id => $teacher)
                        <option value="{{ $id }}" {{ ($id==$schoolClass->teacheraide_id)?'selected':'' }}>{{ $teacher }}</option>
                    @endforeach
                </select>
                <span class="help-block">{{ ''}}</span>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name='template_id[]' class="custom-select select2" multiple data-maximum-selection-length="2">
                            @foreach($templates as $id => $template)
                                <option value="{{ $id }}" {{ (in_array($id, old('template_id', [])) || $schoolClass->schedule_template->contains($id))  ? 'selected' : '' }}>{{ $template }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name='level_type' class="custom-select select2">
                            @foreach($groups as $id => $group)
                                <option value="{{ $group }}" {{ $group==$schoolClass->level_type ? 'selected':'' }}>{{ $group }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
...
<script>
    $(document).ready(function(){
        ...
        $('select[name="campus"]').on("change", function(e){
           ...
           // window.location= "{{ route("admin.school-classes.edit",[$schoolClass->id]) }}&campus=" + $(this).val();
