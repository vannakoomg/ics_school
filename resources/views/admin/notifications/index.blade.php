                ...
                <tbody>
                   ...
                   <tr>
                        ...
                        <td>
                            {{ $message->date }}
                        <br/>
                           <span class="text-muted">{{ $message->time }}</span> </td>
                        <td>
                            <div class="xzoom-thumbs text-left">
                                <img src="{{ asset('storage/image/' . $message->thumbnail) }}" class="btn btn-outline-primary xzoom-gallery img-thumbnail" id="img_thumbnail" xpreview="{{ asset('storage/image' . $message->thumbnai) }}" style="height: 100px; padding:0 !important;margin:0px">
                            </div>
                        </td>
                        <td>{{ $message->title }}</td>
                        <td>{!! $message->body !!}</td>
                        ...
                        <td>
                                    ...
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.message.show', $message->id) }}">
                                        {{ trans('global.view') }}
                                    </a> 
                                    ...
                                    <form action="{{ route('admin.message.destroy', $message->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
